...
import '../../../widgets/atoms/cards/answer_card.dart';
import '../../../widgets/organisms/app_bars/none_title_appbar.dart';
import 'completed_test_page.dart';
      ...
      if (_index < _list!.length) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                _list![_index].vocabulary,
                style: Theme.of(context).textTheme.displayLarge,
              ),
              Column(
                children: [
                  AnswerCard(
                      answer: answer[0].keys.first,
                      onTap: () => nextQuestion(
                          answer[0].keys.first, answer[0].values.first)),
                  const SizedBox(
                    height: 15,
                  ),
                  AnswerCard(
                    answer: answer[1].keys.first,
                    onTap: () => nextQuestion(
                        answer[1].keys.first, answer[1].values.first),
                  ),
                  const SizedBox(
                    height: 15,
                  ),
                  AnswerCard(
                      answer: answer[2].keys.first,
                      onTap: () => nextQuestion(
                          answer[2].keys.first, answer[2].values.first)),
                  const SizedBox(
                    height: 15,
                  ),
                  AnswerCard(
                    answer: answer[3].keys.first,
                    onTap: () => nextQuestion(
                        answer[3].keys.first, answer[3].values.first),
                  )
                ],
              ),
            ],
