...
class AddVocabularyScreen extends ConsumerWidget {
          ...
          child: Column(
            ...
            children: [
              Column(
                children: [
                  CustomTextField(
                      controller: vocabularyController, name: "Vocabulary"),
                  const SizedBox(
                    height: 10,
                  ),
                  CustomTextField(controller: meanController, name: "Meaning"),
                ],
              ),
  ...
  List<Widget> _buildListVocabulary(
    ...
    for (var vocabulary in listVocabulary) {
      vocabularyComponents.add(Stack(
        children: [
          Container(
            decoration: BoxDecoration(
          ...
          Align(
            ...
            child: IconButton(
              ...
              onPressed: () {
                ref.read(listVocabularyNotifierProvider.notifier).deleteVocabulary(listVocabulary.indexOf(vocabulary));
                },
            ),
          )
        ],
      ));
      vocabularyComponents.add(const SizedBox(height: 5));
    }
    ...
    return Scaffold(
      appBar: NonTitleAppBar(
        ...
        actions: [
          TextButton(
              onPressed: () async {
                ref.read(createLessonNotifierProvider.notifier).setListLearning(listVocabulary);

                final newLesson = ref.watch(createLessonNotifierProvider);

                CreateVocabularyLessonDTO lesson = CreateVocabularyLessonDTO(title: newLesson.title??"", courseId: courseId,listVocabulary: newLesson.listVocabulary);
